...
#rmarkdown::render(input = "/dmine/code/git/paper-PHD1/appendix/paper-PHD1-appendix_PCA.Rmd", output_file = paste("/tmp/seamon_PHD_appendix_B.html", sep=""))
...
library(FactoMineR)
result <- FactoMineR::PCA(t(RMA_damage_PNW_exogenous_wheat_year_factor)) # graphs generated automatically
...
PC_table_year <- cbind(result$var$coord[,1], result$var$coord[,2])
colnames(PC_table_year) <- c("PC1", "PC2")
...
library(FactoMineR)
result <- FactoMineR::PCA(t(RMA_damage_IPNW_exogenous_factor), scale.unit = TRUE) # graphs generated automatically
...
PC1_map <- as.data.frame(result$var$coord[,1])
...
PC2_map <- as.data.frame(result$var$coord[,2])
...
map <- leaflet(data = PC2_map, options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.1)) %>%  
  ...
  fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(fillColor = ~pal(PC2_map$PC2),  fillOpacity = .8, weight = 1, stroke = TRUE, smoothFactor = 0.5, color = "black") %>% addPolygons(data = states, fillOpacity = 0, weight = 4, stroke = TRUE, smoothFactor = 0.5, color = "black") %>% 
    ...
    addLegend(pal = pal, values = na.omit(~PC2), group = "circles", title = paste("PC2 Variable Loadings", sep = ""), na.label = "", position = "topright") %>%
...
library(FactoMineR)
result <- FactoMineR::PCA(RMA_damage_IPNW_exogenous_factor, scale.unit = TRUE) # graphs generated automatically
