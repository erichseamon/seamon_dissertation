...
#rmarkdown::render(input = "/dmine/code/git/paper-PHD1/appendix/paper-PHD1-appendix_EDA.Rmd", output_file = paste("/tmp/seamon_PHD_appendix_A.html", sep=""))
...
ggplot(WHEAT_damage_top_decline_moisture) + 
    geom_bar( aes(fill=damagecause, y=loss, x=year), stat="identity") + theme_bw() + theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank()) + ggtitle("IPNW wheat loss top damage causes \nvs. wheat production") + theme(axis.title.y = element_text(family = "Serif", size=24), axis.title.x = element_text(family = "Serif", size = 24), axis.text.x = element_text(size=rel(2.5), angle = 90, hjust = 1, family = "Serif"), axis.text.y = element_text(size=rel(2.5), hjust = 1, family = "Serif")) + theme(plot.title = element_text(size =28, family = "Serif"))  +  scale_x_discrete(name = "Years", limits=c(2001:2015)) + theme(legend.text=element_text(family = "Serif", size=14)) + scale_fill_manual("legend", values = c("Drought" = "red", "Heat" = "darkred", "Excessive Moisture" = "blue", "Decline in Price" = "gray")) + theme(legend.title=element_text(family = "Serif", size=24, face = "bold")) + theme(plot.title = element_text(size=24, family = "Serif", face = "bold")) + scale_y_continuous(name = "Wheat Production/Insurance claim loss ($)", label = unit_format(unit = "M", scale = 1e-6, digits = 0)) + theme(legend.position="bottom") + geom_line(data=iPNW_wheat_yields_annual_average_barplot, aes(x=Year, y=ave_yield), colour = "blue")
