...
In order to generate any of the following tables and graphs, we download all data locally to your /tmp directory, from the seamon_dissertation_dataload.R file from the following location:
...
download.file("https://raw.githubusercontent.com/erichseamon/seamon_dissertation/master/data/seamon_dissertation_dataload.R", destfile = "/tmp/dataload.R")

usePackage("RCurl")
source("/tmp/dataload.R", local=FALSE)
...
wheatprice <- read.csv("/tmp/wheatprice.csv", header=TRUE, strip.white =TRUE)
colnames(wheatprice) <- c("ID", "year", "month","price")
wheatprice_all <- subset(wheatprice, year >= 2001 & year <= 2015)

cpi <- data.frame(read.csv("/tmp/CPI.csv", header = TRUE, strip.white = TRUE))
...
wheatproduction <- read.csv("/tmp/wheatproduction.csv", header=TRUE, strip.white =TRUE)
...
WA_wheat_yields <- read.csv("/tmp/Washington_yields_winterwheat.csv", header = TRUE)
...
ID_wheat_yields <- read.csv("/tmp/Idaho_yields_winterwheat.csv", header = TRUE)
...
OR_wheat_yields <- read.csv("/tmp/Oregon_yields_winterwheat.csv", header = TRUE)
